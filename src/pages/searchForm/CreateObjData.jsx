...
const CreateObjData = () => {
  ...
  const { OBJ_ID, main_obj } = location.state;
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      for (const ele of fList) {
        if (
          ele.DATA_TYPE === "Dynamic Lookup" ||
          ele.DATA_TYPE === "Dynamic Lookup Multiple Choice"
        ) {
          options = await getOptions({ FIELD_ID: ele.FIELD_ID });
  ...
  return (
    <Container maxWidth="xl" key="container">
      ...
      <div style={{ backgroundColor: "#eee" , display:"flex",justifyContent:"center" }}>
        <Typography variant="h5"> OBJECT ID:{main_obj}</Typography>
      </div>
